class Producto {
    ...
    drawCard() {
        ...
        imagen.setAttribute("src", this.portada[0]);
    ...
    drawVentModal() {
        ...
        imgContainer.setAttribute("class", "col-5 col-xl-3 p-0");

        const imgContainerRow = document.createElement("div");
        imgContainerRow.setAttribute("class", "row");

        const btnPrevious = document.createElement("button");
        btnPrevious.setAttribute("class", "col-2 col-lg-1 btn-transp btn-prev");
        btnPrevious.setAttribute("type", "button");

        const btnNext = document.createElement("button");
        btnNext.setAttribute("class", "col-2 col-lg-1 btn-transp btn-next");
        btnNext.setAttribute("type", "button");

        const imgContainerCol = document.createElement("div");
        imgContainerCol.setAttribute("class", "col-8 col-lg-10");

        const imageLibro = document.createElement("img");
        imageLibro.setAttribute("class", "img-fluid d-block m-auto");
        imageLibro.setAttribute("src", this.portada[0]);
        imageLibro.setAttribute("alt", `Una foto relacionada con el libro ${this.nombre}`);
        ...
        imgContainer.append(imgContainerRow, precio);
        imgContainerRow.append(btnPrevious, imgContainerCol, btnNext);
        imgContainerCol.append(imageLibro);

        const listContainer = document.createElement("div");
        listContainer.setAttribute("class", "col-7 col-xl-5");
        ...
        mediosPago.setAttribute("class", "col-12 col-xl-4 border border-secondary rounded-3");
        ...
        document.addEventListener("keydown", (e) => {
            if (e.key === "ArrowLeft") {
                index--;
                if (index < 0) { index = this.portada.length - 1; }
            }
            else if (e.key === "ArrowRight") {
                index++;
                if (index === this.portada.length) { index = 0; }
            }
            else {
                return;
            }
            imageLibro.setAttribute("src", this.portada[index]);
        });
    ...
    static closeProductoModal() {
        if (document.querySelector(".modal-libro")) {
