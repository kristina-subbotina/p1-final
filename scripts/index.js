...
fetch("./libros.json")
    ...
    .then(json => {
        ...
        catalogo.carrito.drawFinalizarModal();
        catalogo.carrito.load();
        catalogo.carrito.drawCheckout();
        banner.draw();
        ...
        // Deteccion del teclado Escape para cerrar 

        document.addEventListener("keydown", (e) => {

            if (e.key === "Escape") {
                catalogo.carrito.closeCarritoModal();
                catalogo.carrito.closeCheckoutModal();
                catalogo.carrito.closeFinalizarModal();
...
// La func para encontrar un producto por ISBN se usa para Call to Action de banner

function productByIsbn(isbn){
    let libros = catalogo.products.filter(libro => libro.isbn === isbn);
    if (libros.length > 0) {
        return libros[0];
    } else {
